...
- 自由工作流中的重执行 Skill 已完成外置，主仓当前只保留 `task_status_skill` / `task_list_skill` 两个只读轻能力。
- MCP Runtime 已完成真实 HTTP 执行、基础重试与最小熔断；自由工作流与专业工作流的主执行分流已收口到 `execution_gateway`，但整个任务主链仍未完全只剩一条正式路径。
- 专业工作流已具备只读触手接入与治理字段，且 runtime 执行入口已统一接入 execution gateway，但审批状态机、人工确认恢复执行、任务真源周边仍未最终收口。
- 触手配置存在文件/路径耦合，未完全收口为统一运行态真源。
- 主脑分层骨架已建立，`brain_core` 对 `master_bot_service` 的直接依赖已清掉，但统一接待、统一路由、统一编排的代码收口还未完成，旧逻辑仍散落在 `message_ingestion_service.py`、`workflow_execution_service.py` 与兼容层边缘。
- `pdf-mcp` 与 `search-mcp` 已有独立目录，但主系统注册与部署链路未完全产品化。

## 2.1 2026-04-14 进度补充

- `brain_core/coordinator` 已成为主消息分发入口。
- `message_ingestion_service.py` 的主分发链路已统一先走 coordinator。
- `master_bot_service.py` 已收缩为兼容层，不再承载真实脑内裁决。
- 新增 `backend/app/execution_gateway/skill_execution_gateway.py`，统一 skill/runtime/builtin 执行分流。
- `free_workflow_service.py` 已改为经由 execution gateway 做 runtime/builtin/fallback/shadow 分流。
- `professional_workflow_service.py` 已改为经由 execution gateway 做严格 runtime 执行。
- `backend/app/brain_core/orchestration/service.py` 已统一生成 `execution_plan_snapshot`。
- `backend/app/brain_core/orchestration/service.py` 已开始承接消息入站任务聚合根装配与 state machine 产出。
- `backend/app/brain_core/manager/service.py` 已开始承接 `manager_packet / brain_dispatch_summary` 的状态刷新，减少入口服务里的项目经理状态胶水。
- `backend/app/brain_core/task_view/service.py` 已开始承接消息入站返回投影（`manager_summary / brain_dispatch_summary`），并已统一承接任务读模型投影（治理字段回填、dispatch_context 回填、失败归因、`status_reason` 聚合）。
- `backend/app/brain_core/orchestration/service.py` 已开始承接 professional confirmation / context patch 的任务与 run 应用补丁，继续压缩 `message_ingestion_service.py` 的状态胶水。
- `backend/app/brain_core/coordinator/service.py` 与 `backend/app/brain_core/orchestration/service.py` 已开始把 `direct_agent_dispatch` 内部主名切到 `agent_dispatch`，并保留旧别名兼容。
- `backend/app/brain_core/task_view/service.py` 已新增 task-event response helper，新任务返回、professional confirmation、context patch 的返回投影继续从入口服务回收到主脑展示层。
- `backend/app/services/dashboard_service.py` 与 `backend/app/services/collaboration_service.py` 已直接改走 `brain_core/task_view/service.py`，任务视图消费面不再依赖旧聚合逻辑。
- `backend/app/services/workflow_execution_service.py` 已开始把 direct-agent fallback 运行语义中性化为 `agent_dispatch` 兼容概念，保留旧命名 wrapper 以避免外部调用断裂。
- `backend/app/brain_core/routing/service.py` 已开始把 `fallbackPolicy.mode` 主名切到 `agent_dispatch_fallback`，执行层兼容旧值 `direct_agent_fallback`。
- `backend/app/brain_core/routing/planner.py` 与 `backend/app/brain_core/routing/service.py` 已优先切到 `resolve_agent_dispatch_execution_agent` 与 `AGENT_DISPATCH_*` 常量，旧命名只保留兼容层用途。
- `backend/app/brain_core/reception/service.py` 已开始承接 active task 解析、context patch 计划生成、professional confirmation transition。
- `backend/app/services/workflow_execution_service.py` 已把 `dispatch / execute / tick` 三套推进逻辑统一到单一推进核心，减少主链分叉。
- `backend/app/services/workflow_execution_service.py` 已把直接 `execute_task` 的异常处理与结果校验统一到单一 helper，减少执行分支重复。
- `backend/app/services/message_ingestion_service.py` 已改用中性的 `create_agent_dispatch_run_for_task`，降低主链对 `direct_agent_fallback` 命名的耦合。
- `backend/app/services/collaboration_service.py` 与协作页已接入执行计划可视化，前端可直接展示 plan/fallback/branch result。
- 架构边界检查已新增“禁止 `brain_core` 重新 import `master_bot_service`”规则。
- 最近一轮主链回归基线：`150 passed`。
...
- `free_workflow_service.py` 与 `professional_workflow_service.py` 已先收口到 `execution_gateway/skill_execution_gateway.py`
- 下一步应继续把 `workflow_execution_service.py` 与 `message_ingestion_service.py` 的主链装配逻辑迁薄

### 6.2 主脑开发顺序（从快到全）

第一阶段：先收口入口，不先重写所有服务

- 新建 `brain_core/coordinator/*`
- 统一消息标准化入口：`ReceptionService`
- 统一路由结果标准化：`RoutingService`
- 统一形成 `BrainDispatchPlan`
- 先让 `message_ingestion_service.py` 调用 coordinator，而不是直接拼装主脑决策

第二阶段：迁移旧主脑逻辑

- 把 `master_bot_service.py` 中的工作流模式识别、治理字段补齐、动态编排计划生成，逐步迁到 `brain_core/routing/*`
- 把 `message_ingestion_service.py` 中的接待判断、上下文续写判断、任务创建前编排，逐步迁到 `brain_core/reception/*` 与 `brain_core/orchestration/*`
- 把任务详情展示聚合逐步迁到 `brain_core/task_view/*`

第三阶段：旧服务瘦身

- `message_ingestion_service.py` 只保留 API 入口编排与状态写入
- `master_bot_service.py` 只保留兼容层，最终改为薄包装或删除
- 主脑内部统一只产出标准 `route_decision` / `execution_plan` / `task_view`

### 6.3 当前主脑代码层职责（明确划分）

- `backend/app/brain_core/reception/*`
  处理入站消息标准化、语言/渠道/用户上下文收口、续写/澄清识别前置。

- `backend/app/brain_core/routing/*`
  处理意图判断、工作流模式判断、治理字段补齐、标准 `route_decision` 产出。

- `backend/app/brain_core/orchestration/*`
  处理执行计划、任务创建前编排、Execution Gateway 请求构建、后续多步计划状态机。

- `backend/app/brain_core/task_view/*`
 处理任务摘要、任务视图、用户可见进度模型聚合，以及任务查询视图的治理字段回填、dispatch_context 回填、失败归因与 `status_reason` 统一投影。

- `backend/app/services/message_ingestion_service.py`
  过渡期 API 入口层。负责安全网关、记忆接入、调用 `brain_core`、写任务与 run 记录。

- `backend/app/services/master_bot_service.py`
  过渡期兼容路由层。现阶段仍承载部分判定逻辑，但目标是逐步迁空。
- `free_workflow_service.py` 内具体技能实现 -> `tentacle_adapters/*` 或外置MCP服务
- `task_service.py`（查询视图段）-> `brain_core/task_view/*`
  当前已收缩为 attach_scope + run/steps 装载兼容层，核心读模型逻辑已迁入 `brain_core/task_view/*`
...
- external registry 不可用时可降级但主链不断；可通过统一开关回滚到 `hybrid`。
