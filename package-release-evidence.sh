...
if ! docker info >/dev/null 2>&1; then
  ...
  echo "Start Docker Desktop or your Docker service first, then rerun ./package-release-evidence.sh"
  exit 1
fi

if ! docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; then
  echo "Backend container '$CONTAINER_NAME' is not running."
  echo "Start the brain first with ./run-brain.sh, then rerun ./package-release-evidence.sh"
  exit 1
fi
...
exec docker exec "$CONTAINER_NAME" sh -lc \
  "cd /app && WORKBOT_REPO_ROOT=$CONTAINER_REPO_ROOT python scripts/package_release_evidence_bundle.py$quoted_args"
