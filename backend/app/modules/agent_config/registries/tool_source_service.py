...
class ToolSourceService:
    def __init__(
        ...
        *,
        ...
        source_scan_mode: str | None = None,
        enable_local_agent_source: bool | None = None,
        enable_local_mcp_source: bool | None = None,
    ) -> None:
        ...
        self._source_scan_mode_override = str(source_scan_mode or "").strip().lower() or None
        self._enable_local_agent_source_override = enable_local_agent_source
        self._enable_local_mcp_source_override = enable_local_mcp_source
    ...
    def _resolve_source_scan_mode(self) -> tuple[str, bool, bool, bool]:
        if self._source_scan_mode_override is not None:
            mode = self._source_scan_mode_override
            if mode not in {"external_only", "hybrid", "local_only"}:
                mode = "hybrid"
            enable_local_agent_source = (
                bool(self._enable_local_agent_source_override)
                if self._enable_local_agent_source_override is not None
                else True
            )
            enable_local_mcp_source = (
                bool(self._enable_local_mcp_source_override)
                if self._enable_local_mcp_source_override is not None
                else True
            )
        else:
    ...
    def _load_external_sources_registry(self) -> list[dict[str, Any]]:
        registry_file = str(os.getenv(EXTERNAL_TOOL_SOURCES_FILE_ENV, "")).strip()
        configured_file_candidates: list[Path] = []
        if registry_file:
            registry_path = Path(registry_file)
            if not registry_path.is_absolute():
                registry_path = PROJECT_ROOT / registry_path
            configured_file_candidates.append(registry_path)

        if registry_file:
            fallback_file_candidates: list[Path] = [*DEFAULT_EXTERNAL_REGISTRY_FALLBACK_PATHS, PROJECT_EXTERNAL_REGISTRY_PATH]
        else:
            fallback_file_candidates = [PROJECT_EXTERNAL_REGISTRY_PATH, *DEFAULT_EXTERNAL_REGISTRY_FALLBACK_PATHS]

        checked_paths: set[Path] = set()
        configured_path = configured_file_candidates[0] if configured_file_candidates else None
        for registry_path in configured_file_candidates:
            if registry_path in checked_paths:
                continue
            checked_paths.add(registry_path)
            try:
        ...
        configured_path = fallback_file_candidates[0] if fallback_file_candidates else configured_path
        for registry_path in fallback_file_candidates:
            if registry_path in checked_paths:
                continue
            checked_paths.add(registry_path)
            try:
                ...
                if registry_path in DEFAULT_EXTERNAL_REGISTRY_FALLBACK_PATHS:
                    logger.warning(
                        "Failed to read external tool sources registry file %s: %s",
                        registry_path,
                        exc,
    ...
    def _scan_internal_skills_source(self) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        from app.modules.dispatch.single_agent_runtime.free_workflow_service import free_workflow_service  # noqa: F401
...
tool_source_service = ToolSourceService(
    source_scan_mode="external_only",
    enable_local_agent_source=False,
    enable_local_mcp_source=False,
)
