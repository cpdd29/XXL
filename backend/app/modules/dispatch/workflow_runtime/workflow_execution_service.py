...
from app.modules.dispatch.execution_support.document_search_service import document_search_service
...
from app.modules.dispatch.workflow_runtime.mandatory_workflow_module_registry_service import foundation_workflow_module_specs
...
from app.platform.persistence.runtime_store import LEGACY_WORKFLOW_IDS as REMOVED_WORKFLOW_IDS, store
...
def _build_security_pipeline_output_result(task: dict, run: dict, node: dict) -> dict[str, Any]:
    ...
    if allowed:
        summary = allowed_message or "已完成安全校验"
        content = allowed_message or "已通过安全校验。"
    else:
        summary = blocked_detail or "请求已被安全拦截"
        content = blocked_detail or "请求已被安全拦截。"
...
def _schedule_message_auto_progress(run_id: str) -> None:
    step_delay = _workflow_step_delay_for_run(run_id)
    configured_delay = min(float(get_settings().message_debounce_seconds), 0.5)
...
def _node_route_key(node: dict | None) -> str | None:
    if not isinstance(node, dict):
        return None
    direct_key = _normalize_node_binding(node.get("route_key") or node.get("routeKey"))
    if direct_key:
        return direct_key.lower()
    config = _node_config(node)
    return _normalize_node_binding(config.get("route_key") or config.get("routeKey"))
...
def _explicit_agent_binding(node: dict | None) -> str | None:
    if not isinstance(node, dict):
        return None
    return _normalize_node_binding(node.get("agent_id") or node.get("agentId"))
...
def _workflow_upstream_result(
    *,
    ...
    if isinstance(graph_state, dict):
        node_results = graph_state.get("node_results")
        if isinstance(node_results, dict):
            ...
            for node_id in reversed(completed_node_ids):
                candidate = node_results.get(node_id)
...
def _select_workflow_for_intent(intent: str | None = None) -> dict:
    ...
    if normalized_intent == "help":
        for workflow in workflows:
            if str(workflow.get("id") or "").strip() == CONVERSATION_AGENT_PIPELINE_WORKFLOW_ID:
                return workflow

    selected_agent_type = INTENT_AGENT_TYPE_MAP.get(normalized_intent)
    ...
    if normalized_intent:
        for workflow in workflows:
            if normalized_intent in _trigger_haystack(workflow.get("trigger")):
                return workflow
...
def _step_node_id(step: dict | None) -> str | None:
    ...
    marker = "nodeid["
    if marker in step_id:
        suffix = step_id.split(marker, maxsplit=1)[-1]
        encoded_node_id, _, _ = suffix.partition("]")
        normalized = str(encoded_node_id or "").strip()
        if normalized:
            return normalized
    return None
...
def _append_node_step(
    *,
    ...
    if node_id:
        step["id"] = f"{task_id}-nodeid[{node_id}]-{uuid4().hex[:8]}"
...
def _resolve_selected_node_execution_agent(
    workflow: dict,
    run: dict,
    intent: str | None,
    *,
    ...
    node = selected_node or _selected_branch_node(workflow, intent)
    if node is not None and _normalize_workflow_node_type(node.get("type")) == "agent":
        explicit_binding = _explicit_agent_binding(node)
        if explicit_binding:
            route_seed = _execution_route_seed(
                run=run,
                workflow=workflow,
                agent_type=_resolve_agent_type(explicit_binding),
            )
            return _resolve_agent_binding(
                explicit_binding,
                expected_type=None,
    ...
    return _resolve_dispatch_execution_agent(workflow, run, intent)
...
def _selected_branch_node(workflow: dict, intent: str | None) -> dict | None:
    ...
    if not selected_agent_type:
        for node in nodes:
            node_type = _normalize_workflow_node_type(node.get("type"))
            if node_type == "workflow" and _derive_workflow_binding(node):
                return node
            if node_type == "tool" and _derive_tool_binding(node):
                return node
            if node_type == "agent" and (
                _derive_agent_binding(node) or _derive_agent_type(node) or str(node.get("label") or "").strip() or _node_route_key(node)
            ):
                return node
        return None

    routed_matches: list[dict] = []
    fallback_matches: list[dict] = []
    for node in nodes:
        node_type = _normalize_workflow_node_type(node.get("type"))
        route_key = _node_route_key(node)
        if node_type == "workflow" and _derive_workflow_binding(node):
            fallback_matches.append(node)
            continue
        if node_type == "tool" and _derive_tool_binding(node):
            fallback_matches.append(node)
            continue
        if node_type not in {"agent", "security-agent", "dialogue-agent", "knowledge-agent", "workflow-router", "handoff-output"}:
            continue
        if route_key == selected_agent_type:
            routed_matches.append(node)
            continue
        if _derive_agent_type(node) == selected_agent_type or selected_agent_type in str(node.get("label", "")).lower():
            fallback_matches.append(node)

    if routed_matches:
        return routed_matches[0]
    if fallback_matches:
...
def _build_search_result(task: dict, run: dict) -> dict:
    ...
    if language == "en":
        bullets = [
            ...
            *[
            ...
            "If you need a deeper pass, follow the task detail logs and rerun with extra context.",
        ]
        if context_notes:
    ...
    bullets = [
        ...
        *[
        ...
        "如需继续深入，可沿着任务详情中的执行日志回溯具体卡点并补充上下文后重跑。",
    ]
    if context_notes:
    ...
    if language == "en":
        ...
        draft = [
            ...
            *(
            ...
            "",
            "Suggested wording:",
                    "1. WorkBot is structured around a unified intake layer, security gateway, Master Bot dispatch, agent orchestration, and visual workflow operations.",
    ...
    if language == "en":
        return {
            ...
            "content": "\n".join(
                ...
                "3. If the issue is still unresolved, move to the task detail view and continue tracing with extra context.",
                ]
            ),
            "bullets": [
...
def _execute_internal_graph_agent_node(
    *,
    ...
    if agent_type == "conversation":
        ...
        bound_agent_id = _derive_agent_binding(node)
        if not bound_agent_id:
            return {"error": "dialogue-agent-missing-binding"}, "对话节点未绑定智能体，无法接管对话"
        ...
        if clarify_required:
            ...
            message = handoff_summary or "对话智能体暂未返回结果，请检查 Agent 绑定与执行配置"
        return {
...
def _is_guard_only_security_agent_node(node: dict | None) -> bool:
    if not isinstance(node, dict):
        return False
    if _normalize_workflow_node_type(node.get("type")) != "agent":
        return False
    resolved_type = str(_derive_agent_type(node) or "").strip().lower()
    resolved_binding = str(_derive_agent_binding(node) or "").strip().lower()
    return resolved_type == "security" or resolved_binding == "security"


def _execute_guard_only_security_agent_node(
    *,
    task: dict,
    run: dict,
    node: dict,
    ...
    if isinstance(current_result, dict):
        candidate_text = str(
            alias_text(current_result, "text", "content", "summary", "title") or ""
        ).strip()
    if not candidate_text:
        candidate_text = str(task.get("description") or task.get("title") or "").strip()
    if not candidate_text:
        candidate_text = _execution_node_label(node, fallback="安全节点检查")

    user_key = (
        str(task.get("user_key") or "").strip()
        or str(dispatch_context.get("user_key") or "").strip()
        or "workflow-security-guard"
    )
    snapshot = security_gateway_service.inspect_text_entrypoint_snapshot(
        text=candidate_text,
        user_key=user_key,
    ...
    verdict = store.clone(snapshot.get("security_verdict") or {})
    guard_summary = {
        "allowed": bool(snapshot.get("allowed")),
        "audit_trace_id": str(snapshot.get("audit_trace_id") or "").strip() or None,
        "warning_count": int(snapshot.get("warning_count") or 0),
    ...
    dispatch_context["security_guard_summary"] = guard_summary

    if not bool(snapshot.get("allowed")):
        blocked_result = {
            "security_blocked": True,
            "security_guard_summary": guard_summary,
            "detail": str(snapshot.get("detail") or "检测到潜在泄密或攻击风险").strip(),
        }
        return blocked_result, "检测到潜在泄密或攻击风险，已紧急停止输出", True

    if isinstance(current_result, dict):
        preserved_result = store.clone(current_result)
        security_context = preserved_result.get("security_context")
        if isinstance(security_context, dict):
            merged_security_context = store.clone(security_context)
        else:
            merged_security_context = {}
        merged_security_context["guard_summary"] = guard_summary
        preserved_result["security_context"] = merged_security_context
        return preserved_result, "已完成安全监测，未发现风险，继续流转", False

    return {"security_guard_summary": guard_summary}, "已完成安全监测，未发现风险，继续流转", False


def _is_conversation_egress_node(node: dict | None) -> bool:
    if not isinstance(node, dict):
        return False
    if _normalize_workflow_node_type(node.get("type")) != "agent":
        return False
    config = _node_config(node)
    module_key = str(config.get("moduleKey") or config.get("module_key") or "").strip().lower()
    return module_key == "conversation_egress"


def _is_passthrough_ready_conversation_result(task_result: dict[str, Any] | None) -> bool:
    if not _is_valid_task_result_payload(task_result):
        return False

    normalized_kind = str((task_result or {}).get("kind") or "").strip().lower()
    if normalized_kind in {"chat_reply", "assistant_reply", "conversation_reply", "final_reply"}:
        return True

    for key in ("text", "content", "summary", "title"):
        if str((task_result or {}).get(key) or "").strip():
            return True
    return False
...
def _build_graph_output_result(task: dict, run: dict, node: dict) -> dict[str, Any]:
    ...
    if _is_professional_agent_workflow(run=run):
        ...
        result = {
            "kind": "error_note",
            "title": "工作流执行失败",
            "summary": "输出节点未收到可用结果，已按严格模式终止",
            "content": "输出节点未收到可用结果，且已关闭兜底生成能力。",
            "text": "输出节点未收到可用结果，且已关闭兜底生成能力。",
            "bullets": [],
            "references": [],
        }
...
def _advance_sequential_workflow_run_locked(
    *,
    ...
    for _ in range(max_iterations):
        ...
        if node_type in {"security-agent", "dialogue-agent", "knowledge-agent", "workflow-router", "handoff-output"}:
            legacy_message = (
                f"检测到旧兼容节点类型「{node_type}」，当前环境已禁用旧兼容执行，请改用新节点类型后重试"
            )
            running_step["status"] = "error"
            running_step["finished_at"] = store.now_string()
            running_step["message"] = legacy_message
            _mark_graph_node_error(graph_state, node, message=legacy_message)
            return _fail_workflow_run_due_execution_target_error_locked(
                task=task,
                run=run,
                steps=steps,
                failure_message=legacy_message,
                failure_stage="dispatch",
            )

        elif node_type == "agent":
            if _is_guard_only_security_agent_node(node):
                guard_result, guard_message, blocked = _execute_guard_only_security_agent_node(
                    task=task,
                    run=run,
                    node=node,
                )
                task_result = guard_result
                if blocked:
                    running_step["status"] = "error"
                    running_step["finished_at"] = store.now_string()
                    running_step["message"] = guard_message
                    _mark_graph_node_error(graph_state, node, message=guard_message)
                    return _fail_workflow_run_due_execution_target_error_locked(
                        task=task,
                        run=run,
                        steps=steps,
                        failure_message=guard_message,
                ...
                agent_type = _derive_agent_type(node) or str(_derive_agent_binding(node) or "").strip().lower()
                agent_binding = _derive_agent_binding(node)
                explicit_agent_binding = _explicit_agent_binding(node)
                if explicit_agent_binding:
                    if _is_conversation_egress_node(node):
                        current_result = _graph_current_or_upstream_result(task, run)
                        if _is_passthrough_ready_conversation_result(current_result):
                            passthrough_result = store.clone(current_result)
                            task_result = passthrough_result
                            passthrough_message = "已复用上游对话结果，跳过回传节点重复生成"
                            running_step["status"] = "completed"
                            running_step["finished_at"] = store.now_string()
                            running_step["message"] = passthrough_message
                            _mark_graph_node_completed(
                                graph_state,
                                node,
                                message=passthrough_message,
                                tokens=0,
                                started_at=running_step.get("started_at"),
                                result=passthrough_result,
                            )
                            execution_agent = None
                    if task_result is not None:
                        ...
                        route_seed = _execution_route_seed(
                            run=run,
                            workflow=workflow,
                            agent_type=_resolve_agent_type(explicit_agent_binding),
                        )
                        ...
                        if failure_run is not None:
                            ...
                            return failure_run
                elif agent_type in {"search", "write", "help", "conversation"} or agent_binding == "general_assistant":
                    ...
                    return _fail_workflow_run_due_execution_target_error_locked(
                        ...
                        steps=steps,
                        failure_message=(
                            f"节点「{_execution_node_label(node, fallback='执行节点')}」绑定类型无效，"
                            "当前已禁用内部兜底执行，请配置可用 Agent 绑定"
                        ),
                        failure_stage="dispatch",
                    )
            ...
            unsupported_message = (
                f"节点「{_execution_node_label(node, fallback='执行节点')}」类型「{node_type or 'unknown'}」不受支持，"
                "当前已关闭兜底执行"
            )
            running_step["status"] = "error"
            running_step["finished_at"] = store.now_string()
            running_step["message"] = unsupported_message
            _mark_graph_node_error(graph_state, node, message=unsupported_message)
            return _fail_workflow_run_due_execution_target_error_locked(
                task=task,
                run=run,
                steps=steps,
                failure_message=unsupported_message,
                failure_stage="dispatch",
            )
...
def _queue_agent_execution_locked(
    *,
    ...
    selected_node = _selected_branch_node(workflow, run.get("intent"))
    execution_agent = _resolve_selected_node_execution_agent(
        workflow,
        run,
        run.get("intent"),
        selected_node=selected_node,
