...
from app.platform.contracts.payload_aliases import alias_text, dispatch_context_from_run, route_decision_from_payload
from app.platform.contracts.agent_protocol import (
...
class WorkflowDispatcherService:
    ...
    def _should_bypass_event_bus(self) -> bool:
        # Current deployment is single-backend process with durable DB queues.
        # Bypassing NATS publish avoids timeout-induced retry jitter while
        # keeping dispatch/execution semantics via persisted jobs.
        return self._event_bus is nats_event_bus
    ...
    def process_tick(self, run_id: str, *, step_delay: float) -> dict | None:
        ...
        try:
            ...
            if execution_publish_succeeded or execution_job_durable:
                ...
                return self._find_run(run_id)
            raise RuntimeError("工作流执行投递失败：执行消息未发布且执行队列未落库")
