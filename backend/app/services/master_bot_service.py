...
PROFESSIONAL_CONFIRM_HINTS = {"确认", "开始", "执行", "继续", "同意", "ok", "confirm", "proceed", "yes"}
PROFESSIONAL_CANCEL_HINTS = {"取消", "不用了", "停止", "驳回", "cancel", "stop", "no"}
...
def _is_professional_workflow_request(normalized_text: str) -> bool:
    if not normalized_text:
        return False

    has_system = _contains_any(normalized_text, PROFESSIONAL_SYSTEM_HINTS)
    has_action = _contains_any(normalized_text, PROFESSIONAL_ACTION_HINTS)
    has_business_data = _contains_any(normalized_text, PROFESSIONAL_DATA_HINTS)
    has_permission_hint = _contains_any(normalized_text, PERMISSION_REQUIREMENT_HINTS)
...
def _classify_workflow_mode(text: str, *, intent: str, interaction_mode: str) -> str:
    normalized_text = _normalize_text(text)
    if not normalized_text:
        return "chat"

    if _is_professional_workflow_request(normalized_text):
        return "professional_workflow"

    if (
        _contains_any(normalized_text, LIVE_INFORMATION_HINTS)
        or _contains_any(normalized_text, FREE_WORKFLOW_FILE_HINTS)
        or _contains_any(normalized_text, FREE_WORKFLOW_CONTENT_HINTS)
    ):
    ...
    if interaction_mode == "chat":
        if _contains_any(
            ...
            return "chat"
        if intent == "help" and _is_direct_question(normalized_text):
            return "chat"
...
def _classify_permission_requirement(text: str, *, workflow_mode: str) -> bool:
    ...
    has_permission_hint = _contains_any(normalized_text, PERMISSION_REQUIREMENT_HINTS)
    has_enterprise_context = _contains_any(
        normalized_text,
...
def _infer_required_capabilities(text: str, *, intent: str, workflow_mode: str) -> list[str]:
    normalized_text = _normalize_text(text)
    ...
    is_live_information_request = _contains_any(normalized_text, LIVE_INFORMATION_HINTS)
    is_file_task = _contains_any(normalized_text, FREE_WORKFLOW_FILE_HINTS)
    is_content_task = _contains_any(normalized_text, FREE_WORKFLOW_CONTENT_HINTS)

    if is_professional:
        capabilities.update({"permission_validation", "secure_tool_execution", "audit_logging"})
        capabilities.add("enterprise_system_access")
        if _contains_any(normalized_text, {"crm", "salesforce", "客户", "customer"}):
            capabilities.add("crm_data_access")
        if _contains_any(normalized_text, {"订单", "order"}):
            capabilities.add("order_data_access")
        if _contains_any(normalized_text, {"查询", "获取", "拉取", "检索", "query", "fetch"}):
            capabilities.add("structured_data_query")
        if _contains_any(normalized_text, {"写入", "更新", "同步", "提交", "update", "sync", "submit"}):
            capabilities.add("system_write_operation")
        if _contains_any(normalized_text, {"导出", "pdf", "export"}):
            capabilities.add("document_export")
        if _contains_any(normalized_text, {"发送", "推送", "邮件", "通知", "send", "push", "email"}):
    ...
    if is_live_information_request:
        ...
        if _contains_any(normalized_text, {"天气", "气温", "温度", "weather", "forecast", "temperature"}):
    ...
    if _contains_any(normalized_text, {"任务列表", "最近任务", "task list"}):
    ...
    if _contains_any(normalized_text, {"演讲", "演讲稿", "发言稿", "speech"}):
        capabilities.add("speechwriting")
    if _contains_any(normalized_text, {"文案", "copy", "copywriting"}):
        capabilities.add("copywriting")
    if intent == "search" or _contains_any(normalized_text, SEARCH_TASK_HINTS):
        capabilities.add("information_retrieval")
    if _contains_any(normalized_text, {"总结", "整理", "summarize"}):
        capabilities.add("summarization")
    if _contains_any(normalized_text, {"翻译", "translate"}):
        capabilities.add("translation")

    return _ordered_capabilities(capabilities)


def _build_workflow_metadata(text: str, *, intent: str, interaction_mode: str) -> dict[str, Any]:
    workflow_mode = _classify_workflow_mode(text, intent=intent, interaction_mode=interaction_mode)
    requires_permission = _classify_permission_requirement(text, workflow_mode=workflow_mode)
    required_capabilities = _infer_required_capabilities(
        text,
        intent=intent,
...
def _parse_schedule_plan(text: str) -> dict[str, Any] | None:
    normalized_text = _normalize_text(text)
    if not normalized_text:
        return None
...
def _governance_metadata(text: str, *, workflow_mode: str) -> dict[str, Any]:
    normalized_text = _normalize_text(text)
    schedule_plan = _parse_schedule_plan(text)
    high_risk_write = workflow_mode == "professional_workflow" and _contains_any(
        normalized_text,
...
def _enrich_route_decision_with_workflow_mode(
    ...
    *,
    ...
    user_visible_workflow_mode = _workflow_mode_label(workflow_mode)
    governance = _governance_metadata(text, workflow_mode=workflow_mode)
...
def _apply_interaction_mode_safety_correction(
    text: str,
    *,
    intent: str,
    interaction_mode: str,
...
class MasterBotService:
    def route_message(
        ...
        *,
        ...
        interaction_mode, reception_mode = _apply_interaction_mode_safety_correction(
            text,
            intent=intent,
            interaction_mode=interaction_mode,
        ...
        if interaction_mode == "chat":
            ...
            route_decision = _enrich_route_decision_with_workflow_mode(
                route_decision,
                text=text,
                intent=intent,
                interaction_mode=interaction_mode,
