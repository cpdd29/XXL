...
from app.platform.contracts.event_protocol import validate_event_envelope
...
class NatsEventBus:
    def __init__(
        ...
        *,
        ...
        self.operation_timeout_seconds = float(
            ...
            else settings.nats_operation_timeout_seconds
        )
    ...
    def publish_json(self, subject: str, payload: dict) -> bool:
        ...
        if subject.startswith("brain."):
            from app.platform.audit.event_journal_service import record_event_publish_attempt
        ...
        try:
            ...
            if published and subject.startswith("brain.") and event_id:
                from app.platform.audit.event_journal_service import mark_event_published
            ...
            if subject.startswith("brain.") and event_id:
                from app.platform.audit.event_journal_service import mark_event_publish_failed
