...
from app.core.brain_payload_fields import alias_bool, alias_text, route_decision_from_task
from app.brain_core.routing.rules import dispatch_intent as default_dispatch_intent
    ...
    def should_context_patch(
        ...
        *,
        ...
        if str(active_task.get("status") or "").strip().lower() not in {"pending", "running"}:
            return False
        if (received_at - last_message_at).total_seconds() > int(message_debounce_seconds):
            return False
