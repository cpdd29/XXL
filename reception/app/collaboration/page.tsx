...
import { ExecutionPlanVisualizer } from "@/components/collaboration/execution-plan-visualizer"
...
function managerSummaryEntries(packet?: ManagerPacket | null) {
  if (!packet) return []
...
export default function CollaborationPage() {
  ...
  const managerEntries = managerSummaryEntries(data?.session.managerPacket)
    ...
    <div className="flex h-full flex-col bg-background">
      <div className="border-b border-border bg-card px-6 py-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">执行过程</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              把任务流转、节点状态、Token 消耗和执行日志放在同一个实时视图里，方便快速复盘执行链路。
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <SelectTrigger className="w-[320px] bg-background">
                <SelectValue placeholder="选择一个任务查看执行过程" />
              </SelectTrigger>
              <SelectContent>
                {data?.tasks.map((task) => (
      ...
      {error ? (
        <div className="px-6 py-4 text-sm text-destructive">
          执行过程加载失败：{error instanceof Error ? error.message : "未知错误"}
        </div>
      ) : null}
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <div className="space-y-4">
              ...
              {data.session.managerPacket ? (
                <Card className="bg-card">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base">项目经理分发</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    ...
                    {data.session.managerPacket.handoffSummary ? (
                      <div className="rounded-xl bg-secondary/40 p-3 text-xs leading-5 text-muted-foreground">
                        {data.session.managerPacket.handoffSummary}
                      </div>
                    ) : null}
                    {data.session.managerPacket.clarifyQuestion ? (
                      <div className="rounded-xl bg-warning/10 p-3 text-xs leading-5 text-foreground">
                        澄清问题：{data.session.managerPacket.clarifyQuestion}
                      </div>
                    ) : null}
                  </CardContent>
                </Card>
              ...
              {data.session.brainDispatchSummary ? (
                <Card className="bg-card">
                  <CardHeader className="pb-3">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center justify-between rounded-xl bg-secondary/40 p-3">
                      <span className="text-sm text-muted-foreground">派发形态</span>
                      <span className="text-sm font-medium text-foreground">
                        {data.session.brainDispatchSummary.dispatchType ?? "--"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between rounded-xl bg-secondary/40 p-3">
                      <span className="text-sm text-muted-foreground">工作流模式</span>
                      <span className="text-sm font-medium text-foreground">
                        {data.session.brainDispatchSummary.workflowMode ?? "--"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between rounded-xl bg-secondary/40 p-3">
                      <span className="text-sm text-muted-foreground">执行代理</span>
                      <span className="text-sm font-medium text-foreground">
                        {data.session.brainDispatchSummary.executionAgent ?? "--"}
                      </span>
                    </div>
                    {data.session.brainDispatchSummary.summaryLine ? (
                      <div className="rounded-xl bg-secondary/40 p-3 text-xs leading-5 text-muted-foreground">
                        {data.session.brainDispatchSummary.summaryLine}
                      </div>
                    ) : null}
                ...
                <Card className="bg-card">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center justify-between rounded-xl bg-secondary/40 p-3">
                      ...
                      <span className="text-sm font-medium text-foreground">
                        {data.session.routeDecision.intent ?? "--"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between rounded-xl bg-secondary/40 p-3">
                      <span className="text-sm text-muted-foreground">工作流</span>
                      <span className="text-sm font-medium text-foreground">
                        {data.session.routeDecision.workflowName ?? "--"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between rounded-xl bg-secondary/40 p-3">
                      <span className="text-sm text-muted-foreground">执行代理</span>
                      <span className="text-sm font-medium text-foreground">
                        {data.session.routeDecision.executionAgent ?? "--"}
                      </span>
                    </div>
                    {data.session.routeDecision.routeMessage ? (
                      <div className="rounded-xl bg-secondary/40 p-3 text-xs leading-5 text-muted-foreground">
                        {data.session.routeDecision.routeMessage}
                      </div>
                    ) : null}
              ...
              {data.session.executionPlan ? (
                <ExecutionPlanVisualizer plan={data.session.executionPlan} />
              ) : null}

              {data.session.fallbackHistory && data.session.fallbackHistory.length > 0 ? (
                <Card className="bg-card">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base">回退历史</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {data.session.fallbackHistory.slice().reverse().map((item) => (
                      <div key={item.id} className="rounded-xl border border-border bg-secondary/20 p-3">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-medium text-foreground">
                            {item.reason} {"->"} {item.resolvedAction}
                          </span>
                          <Badge variant="outline" className="border-border text-muted-foreground">
                            {item.failureStage}
                          </Badge>
                        ...
                        <p className="mt-2 text-[11px] leading-5 text-muted-foreground">
                          policy={item.policyMode ?? "--"} · target={item.policyTarget ?? "--"} · at={item.timestamp}
                        </p>
                      </div>
                    ))}
                  </CardContent>
