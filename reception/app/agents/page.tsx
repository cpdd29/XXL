      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border border-border bg-secondary/30 p-3">
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-muted-foreground">本地配置</span>
            <Badge
              variant="secondary"
              className={cn(
                "text-xs",
                agent.configSummary?.status === "loaded" && "bg-success/15 text-success",
                agent.configSummary?.status === "partial" && "bg-warning/20 text-warning-foreground",
                (!agent.configSummary || agent.configSummary?.status === "missing") &&
                  "bg-destructive/15 text-destructive",
              )}
            >
              {agent.configSummary?.status ?? "missing"}
            </Badge>
          </div>
          <div className="mt-2 text-xs text-muted-foreground">
            目录: {agent.configSummary?.directory ?? "--"}
          </div>
          <div className="mt-1 text-xs text-muted-foreground">
            已加载: {agent.configSummary?.filesLoaded?.join(", ") || "--"}
          </div>
