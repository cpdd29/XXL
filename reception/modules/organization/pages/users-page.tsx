...
import { downloadUsers, useUserTenants, useUsers } from "@/modules/organization/hooks/use-users"
import { toast } from "@/shared/hooks/use-toast"
import { cn } from "@/shared/utils"
import type { UserPortrait } from "@/shared/types"
...
export default function UsersPage() {
  ...
  const tenantOptions = useMemo(() => tenantOptionsData?.items ?? [], [tenantOptionsData?.items])
  const tenantPortraits = useMemo(() => tenantPortraitsData?.items ?? [], [tenantPortraitsData?.items])
  const portraits = useMemo(() => data?.items ?? [], [data?.items])
  const selectedTenantMeta = useMemo(() => {
    ...
    return tenantOptions.find((tenant) => tenant.id === selectedTenant) ?? null
  }, [selectedTenant, tenantOptions, tenantPortraits.length])
